...
export const FiberBadge = forwardRef<HTMLDivElement, NodeProps>(({
  ...
  const suffix10 = data ? <SVGData key="data" title="Data" /> : null;
  const suffix11 = ooo ? '⚠️' : null;

  const icons = [suffix1, suffix2, suffix3, suffix4, suffix5, suffix6, suffix7, suffix8, suffix9, suffix10, suffix11].filter(x => !!x);

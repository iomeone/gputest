// 文件：src/fix-wgsl.mjs
// 用法：node fix-wgsl.mjs [--dry] [--verbose]
...
// 仅处理相对/本地导入（以 ./ ../ 或 wgsl/ 起始），且路径中包含 /wgsl/ 段，且以 .wgsl 结尾
function shouldTransform(spec) {
  if (!/^(?:\.{1,2}\/|wgsl\/)/.test(spec)) return false;     // 排除别名/包名
  if (!/(^|\/)wgsl\//.test(spec)) return false;              // 必须有 wgsl 目录段
  if (!/\.wgsl$/.test(spec)) return false;                   // 必须以 .wgsl 结尾
  return true;
}

function transformSpecifier(spec) {
  // 目录 wgsl -> gen-wgsl；去掉 .wgsl 后缀
  return spec.replace(/(^|\/)wgsl\//, '$1gen-wgsl/').replace(/\.wgsl$/, '');
}

...
import type { Blending, OffscreenRenderContext, ColorSpace, RenderViewAttachment, TextureSource, TextureTarget } from '../../core';

import { provide, fence, yeet, useContext, useMemo, useOne } from '../../live';
import { countMips, getTextureSampleType } from '../../core';
...
export const RenderCubeTarget: LiveComponent<RenderCubeTargetProps> = (props: RenderCubeTargetProps) => {
  ...
  const [renderTexture, resolveTexture, bufferTextures, bufferViews, bufferLayers, counter] = useMemo(
    ...
    [device, width, height, format, samples, history, mips, label]
  );
  ...
  const [source, depth] = useMemo(() => {
    ...
    const size = [width, height, 6] as [number, number, number];
    ...
    return [source, depth];
  }, [targetTexture, depthTexture, width, height, format, variant, absolute, samples, history, mips, sampler, depthStencil, hint, bufferLayers, bufferTextures, bufferViews, colorSpace, counter, resolveTexture, viewAttachments]);
